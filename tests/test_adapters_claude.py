...
class ClaudeAdapterTests(unittest.TestCase):
    ...
    def test_session_start_emits_unicode_context_as_utf8_bytes(self) -> None:
        ...
        self.assertEqual(self.strict_utf8(raw), expected)
    ...
    def test_unicode_block_cannot_become_unparseable_success(self) -> None:
        ...
        with mock.patch.object(
            claude,
            "_policy_reason",
            return_value="evidline BLOCK: review café ✓ 漢字",
        ):
            code, raw, stderr = self.invoke_bytes(
                "pre-tool-use",
                self.tool_payload(target="src/café.py"),
                stdout=output,
            )
    ...
    def test_denial_transport_completes_multiple_short_writes(self) -> None:
        ...
        code, raw, stderr = self.invoke_bytes(
            "pre-tool-use",
            self.tool_payload(target=".git/config"),
            stdout=output,
        )
    ...
    def test_denial_encoding_failure_emits_ascii_safe_denial(self) -> None:
        with mock.patch.object(claude, "_policy_reason", return_value="deny \ud800"):
            code, raw, stderr = self.invoke_bytes(
                "pre-tool-use",
                self.tool_payload(target=".git/config"),
            )
    ...
    def test_session_start_transport_failure_is_nonzero(self) -> None:
        output = BinaryOutput(fail_binary_writes=None)
        ...
        self.assertEqual((code, raw), (2, b""))
        self.assertIn(b"session-start output failed", stderr)
    ...
    def test_structured_stdout_write_failure_exits_two(self) -> None:
        ...
        with (
            mock.patch("sys.stdin", stdin),
            mock.patch("sys.stdout", BinaryOutput(fail_binary_writes=None)),
            mock.patch("sys.stderr", stderr),
        ):
    ...
    def test_missing_and_unknown_adapter_commands_exit_two(self) -> None:
        for arguments in ((), ("unknown",), ("session-start", "extra")):
            with self.subTest(arguments=arguments):
                ...
                self.assertEqual((code, stdout.getvalue()), (2, b""))
